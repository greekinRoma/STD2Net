...
class PatchMerging(nn.Module):
    def __init__(self,dim,out_dim,input_resolution):
        ...
        self.conv3 = Conv2d_BN(hid_dim,out_dim,1,1,0,resolution=input//2)
    def forward(self,x):
        x = self.conv1(x)
        x = self.act(x)
        x = self.conv2(x)
...
class CascadedGroupAttention(nn.Module):
    def __init__(self,dim,key_dim,num_heads=8,attn_ratio=4,resolution=14,kernels=[5,5,5,5],**kwargs):
        ...
        for p1 in points:
            for p2 in points:
                offset = (abs(p1[0] - p2[0]), abs(p1[1] - p2[1]))#计算相对偏移量
                if offset not in attention_offsets:
                    attention_offsets[offset] = len(attention_offsets)#对应相对偏移量对应的当前的字典大小
                idxs.append(attention_offsets[offset])#将对应的索引添加到对应的偏移量中，idxs对应的每对偏移量对应的索引值
        self.attention_biases = nn.Parameter(
            torch.zeros(num_heads, len(attention_offsets)))#寻找每个heads的对应数值[num_heads, len(attention_offsets)]
    ...
    def forward(self,x):
        ...
        for i,qkv in enumerate(self.qkvs):
            ...
            feat = (v@attn.tranpose(-2,-1)).view(B,self.d,H,W)
...
class LocalWindowAttention(nn.Module):
    def __init__(self,dim,key_dim,num_heads=8,
        ...
        self.wondow_attn = window_resolution
...
class EfficientViTBlock(nn.Module):
    def __init__(self,type,
        ...
        self.ffn0 = Residual(FFN(ed,int(ed*2),resolution=resolution))
        if type == 's':
